...
The default setting is adjusted for quick run (parameter `quick = TRUE`). Linting and cleansing suggestions will therefore not be executed but packages will be loaded (`library()`). Add/change the logical argument to `quick = FALSE` to install missing packages, execute linting, identify superfluous packages and to get notified, which packages can be removed from the environment:
